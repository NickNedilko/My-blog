...
import { useDeleteUserMutation } from "../../mutations/user-mutations";
         ...
         <Table>
          ...
          <TableBody className="divide-y">
              ...
              <TableRow key={user._id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                          ...
                          <TableCell>
                  <img src={user.avatarUrl} alt={user.userName} className="w-20 h-20 object-cover rounded-full" />   
                </TableCell>
                <TableCell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">
                  {user.userName}  
                            </TableCell>
                <TableCell>{user.email}</TableCell> 
                <TableCell>{user.isAdmin? <FaCheck className="text-green-500 w-5 h-5"/>  : <IoClose className="text-red-500 w-6 h-6"/>}</TableCell>
                <TableCell>
            <button onClick={() => mutate(user._id)}
          ...
          {!data || data?.totalUsers > limit && (
            ...
            <Button
              className="text-white font-semibold py-2 px-4 rounded bg-gradient-to-r from-blue-500 to-pink-500 
                transition-all duration-300  hover:from-pink-500 hover:to-blue-500"
              outline onClick={() => setPage((prev) => prev + 1)} disabled={!data || page * limit >= data?.totalUsers}>
                Next
              </Button>
            <p>Page {page}</p>
          </div>
